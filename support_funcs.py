...
def purify_song_name(song_name, purification_params):
    if purification_params["purification_mode"] == PurificationMode.split_by_symbol:
        song_name = song_name.split(purification_params["split_symbol"])[
            purification_params["split_index"]].split()
        ...
        song_name = song_name.split(
        )[purification_params["left_end"]: purification_params["right_end"]]
...
def get_files_from_directory(song_dir_name):
    ...
    song_files = [splitext(f) for f in listdir(
        song_dir) if isfile(join(song_dir, f))]
...
def apply_params_to_songs(song_files, params):
    for i in song_files:
        if i[1] in SUPPORTED_EXTENTIONS:
            ...
            params["song_params"]["title"] = purify_song_name(
                i[0], params["purification_params"])

            change_song_attributes(params["song_params"])

            if params["rename_files"]:
                rename(params["song_params"]["filename"],
                        params["song_params"]["title"] + i[1])
